...
/**
 * Activate an approved offer — transitions it from approved → active in Hub.
 * Calls PUT /api/hub/offers/{id}/status?new_status=active.
 */
export async function activateOffer(
  ...
  try {
    const response = await fetch(
      `${SERVER_API_BASE}/api/hub/offers/${offerId}/status?new_status=active`,
      {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
...
/**
 * Expire an offer — transitions it from approved/active → expired in Hub.
 * Calls PUT /api/hub/offers/{id}/status?new_status=expired.
 */
export async function expireOffer(
  ...
  try {
    const response = await fetch(
      `${SERVER_API_BASE}/api/hub/offers/${offerId}/status?new_status=expired`,
      {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
    ...
    if (!response.ok) {
      ...
      const error =
        typeof detail === 'string' && detail.length < 200
          ? detail
          : `Expiration failed (${response.status})`;
      return { success: false, error };
    }

    return { success: true, message: 'Offer expired' };
  } catch {
    return { success: false, error: 'Failed to connect to Hub API' };
  }
}
